...
export default function Questions({ score, setScore, questIndex, setQuestIndex, disable, setDisable }) {

    const [userChoice, setUserChoice] = useState(null);
    const [nextBtn, setNextBtn] = useState(localStorage.getItem('nextBtn') || false);
    ...
    const submitAnswer = () => {
        if(!userChoice){
            ...
            localStorage.setItem('nextBtn', nextBtn);
    ...
    return (
        <>
            <Header />
            <div className="text-center text-2xl font-bold p-5 bg-green-500 text-white ">QUESTION {Number(questIndex) + 1}</div>
            ...
            <div className='flex justify-center gap-2'>
                <button onClick={submitAnswer} className={`${disable ? 'hidden':'flex'} text-[16px] bg-blue-500 px-3 py-2 text-white rounded justify-self-center mt-3 cursor-pointer`}>Submit</button>
